...
contract Brewlabs is ERC20, Ownable {
	using SafeMath for uint256;

	IUniswapV2Router02 public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private _name = "Brewlabs";
	string private _symbol = "BREWLABS";
	...
	CustomTaxPeriod private _launch2 = CustomTaxPeriod('launch2',0,3600,3,5,3,20,1,2,1,3,1,1,4,10);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "Brewlabs: The dividend tracker already has that address");
		BrewlabsDividendTracker newDividendTracker = BrewlabsDividendTracker(payable(newAddress));
		require(newDividendTracker.owner() == address(this), "Brewlabs: The new dividend tracker must be owned by the Brewlabs token contract");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "Brewlabs: Automated market maker pair is already set to that value");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "Brewlabs: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTransactionLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "Brewlabs: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransferChange(account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "Brewlabs: Account is already the value of 'excluded'");
	...
	function blockAccount(address account) public onlyOwner {
		...
		if (_isLaunched) {
			require(currentTimestamp.sub(_launchStartTimestamp) < _blockedTimeLimit, "Brewlabs: Time to block accounts has expired");
		}
	...
	function setWallets(address newLiquidityWallet, address newMarketingWallet, address newDevWallet, address newBuyBackWallet) public onlyOwner {
		...
		if(marketingWallet != newMarketingWallet) {
			require(newMarketingWallet != address(0), "Brewlabs: The marketingWallet cannot be 0");
	...
	function setUniswapRouter(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "Brewlabs: The router already has that address");
	...
	function setGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "Brewlabs: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "Brewlabs: Cannot update gasForProcessing to same value");
	...
	function setMaxWalletAmount(uint256 newValue) public onlyOwner {
		require(newValue != maxWalletAmount, "Brewlabs: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) public onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "Brewlabs: Cannot update minimumTokensBeforeSwap to same value");
	...
	function setDividendToken(address newDividendToken) external onlyOwner {
		require(newDividendToken != dividendToken, "Brewlabs: Cannot update dividend token to same value");
		require(newDividendToken != address(0), "Brewlabs: The dividend token cannot be 0");
		require(newDividendToken != address(this), "Brewlabs: The dividend token cannot be set to the current contract");
		...
		uint256 amount
			...
			if(from != owner() && to != owner()) {
				...
				if (_isInLaunch && currentTimestamp.sub(_launchStartTimestamp) <= 300 && isBuyFromLp) {
					require(currentTimestamp.sub(_buyTimesInLaunch[to]) > 60, "Brewlabs: Cannot buy more than once per min in first 5min of launch");
				}
				if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
					require(amount <= maxTxAmount, "Brewlabs: Buy amount exceeds the maxTxBuyAmount.");
				}
				if (!_isExcludedFromMaxWalletLimit[to]) {
					require(balanceOf(to).add(amount) <= maxWalletAmount, "Brewlabs: Expected wallet amount exceeds the maxWalletAmount.");
				}
			}
...
contract BrewlabsDividendTracker is DividendPayingToken {
	...
	constructor() public DividendPayingToken("Brewlabs_Dividend_Tracker", "Brewlabs_Dividend_Tracker") {
		claimWait = 3600;
	...
	function setTokenBalanceForDividends(uint256 newValue) external onlyOwner {
		require(minimumTokenBalanceForDividends != newValue, "Brewlabs_Dividend_Tracker: minimumTokenBalanceForDividends already the value of 'newValue'.");
		minimumTokenBalanceForDividends = newValue;
	}
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "Brewlabs_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "Brewlabs_Dividend_Tracker: Cannot update claimWait to same value");
